...
One of the nicer theorems concerning completions tells us, among other things, that if we have a regular point $p$ on a variety $X$, then formally locally the variety looks like an affine space near the point $p$, i.e., $\widehat \OO_{X,p} \cong K[[x_1,...,x_n]]$. Before proving this, however, we need a lemma.
...
\end{lem}
\begin{proof}
We reduce this to another problem. Assume that $\gr_I (A)$ is not the free $A/I$-algebra, i.e., we have some homogeneous polynomial $F \in (A/I)[x_1,...,x_r]$ of degree $n$ such that $F([a_1], ... , [a_r]) = 0$. We may lift the polynomial $F$ to a polynomial $\widetilde F \in A[x_1,...,x_r]$ such that $\widetilde F (a_1,...,a_r) \in I^{n+1}$, and hence by Noetherianity of $A$ we may find a nontrivial polynomial $f \in A[x_1,...,x_r]$ \emph{with coefficients outside the ideal $I$} such that $f(a_1,...,a_r) = 0$. We will prove that the existence of this $f$ is impossible.

Assume that $r = 1$. As $a_1$ is not a zerodivisor, we may assume that $f$ has nonzero constant coefficient. As the constant coefficient does not lie in $I$, but each other term of the polynomial $f(a_1)$ does lie in $I$, we see that $f(a_1)$ cannot be zero, so we can conclude the impossibility of the existence of $f$ in the case $r=1$.

Assume then that $r>1$ and all the smaller cases have been taken care of. Again, as $a_1$ is not a zerodivisor, we may assume that $f$ is not divisible by $x_1$. Therefore we obtain a nontrivial polynomial $f(0,x_2,...,x_r) \in (A/(a_1))[x_2,...,x_r]$ with coefficients lying outside the image of $I$, which is an ideal generated by a regular sequence of $r-1$ elements. The existence of such polynomial is however known to be impossible by induction, so we are done.
\end{proof}
...
\begin{rem}
It is clear that the procedure used in the above proof can be generalized to very general situations when trying to find out a representation for a finitely generated sub $A$-algebra of $A[x_1,...,x_n]/I$. However, if $A$ is not an UFD, then there are some technical difficulties with Buchberger's criterion, as there may not be just one ''elimination polynomial'' $h$. In the proof above, this wasn't a problem, as every leading term had either a unit or a prime coefficien, but in more general situations we might run into trouble. Once these technical difficulties are settled, however, one would be able to, for example, find a representation for \emph{any} Rees-algebra $A[It]$ as a quotient ring of some $A[x_1,...,x_n]$ in an entirely mechanical fashion, and hence be able to compute all blow-ups mechanically.
\end{rem}
...
For a $k$-variety $X$, denote by $\der_k(X)$ the (clearly quasicoherent) sheaf of $k$-derivations $\OO_X \to \OO_X$. This sheaf comes with the natural derivation map $\der_k (X) \times \OO_X \to \OO_X$, which is $k$-bilinear. For an ideal sheaf $\mathscr{I} \subset \OO_X$ define the ideal sheaf $D(\mathscr{I})$  by taking the ideal sheaf locally generated by the local images of $\der_k (X) \times \mathscr{I}$ under the derivation map. We can define \emph{higher ideal derivations} $D^n(\mathscr{I})$ in the obvious way by composing $D$ with itself. We can define $\der_k(A)$ and $D^n (I)$ for a $k$-algebra $A$ and an ideal $I$ in an obvious way.

Our next step is to show that the ideal derivation operator $D$ drops the order of the ideal by $1$ at all points (except if the order was already 0). To prove this, we need to find out how $D$ interacts with completion.
