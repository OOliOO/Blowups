...
Our next step is to show that the ideal derivation operator $D$ drops the order of the ideal by $1$ at all points (except if the order was already 0, of course) when $X$ is nonsingular. By passing into local coordinates, we may assume that it is the spectrum of $A=k[x_1,...,x_n] / I$, so that $\der_k(A)$ (which has the obvious meaning) is generated by $\partial_1,...,\partial_n$. If an ideal $J \subset A$ is generated by $[f_1], ..., [f_r]$, then the derivative $D(J)$ is generated by $([\partial_j f_i])_{i,j}$. This construction clearly commutes with extension of base field, and as so does the order of the ideal, we see that it is enough to consider the case when $k$ is algebraically closed.
