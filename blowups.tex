...
Assume $X$ is a smooth $k$-variety. By passing into local coordinates, we may assume that it is the spectrum of $A=k[x_1,...,x_n] / I$, so that $\der_k(A)$ (which has the obvious meaning) is generated by $\partial_1,...,\partial_n$. If an ideal $J \subset A$ is generated by $[f_1], ..., [f_r]$, then the derivative $D(J)$ is generated by $([\partial_j f_i])_{i,j}$. This construction clearly commutes with extension of base field. Also, in our case, it is easy to see that completion in some sense commutes with extension of scalars.

\begin{lem}
Let $A$ be a \emph{regular local} $k$-algebra, with $\kappa = A / m_A$ of degree $n$ over $k$. Now $\overline{k} \otimes_k \widehat{A}$ is naturally isomorphic to $\widehat{(\overline{k} \otimes A)}$, where the later completion is the $\overline{k} \otimes_k I$-adic one.
\end{lem}
