...
\end{equation*}
is surjective, and therefore we may conclude, using dimensionality, that $K[a_1,...,a_n] / J^i \to \widehat{A} / \widehat{m_A}^i$ must be surjective for all $i$. Therefore the image of $K[a_1,...,a_n]$ is dense in $\widehat{A}$, and $\widehat{(K[a_1,...,a_n])} = \widehat A$. As $K[a_1,...,a_n]$ is a finitely generated $K$-algebra, its $J$-adic completion is of the wanted form, and we are done.
\end{proof}
