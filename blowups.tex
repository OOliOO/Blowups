...
Let $A$ be a ring, and let $I \subset A$ be an ideal generated by a regular sequence $a_1, ... , a_r$. Now the associated graded polynomial $\gr_I (A)$ is isomorphic to the polynomial algebra $(A/I) [x_1,...,x_r]$ via the map $x_i \mapsto [a_i]$.
\end{lem}
\begin{proof}
If $r=1$, this is easy. As $a \equiv a_1$ is not a zerodivisor, neither is $a^n$ for any $n$, and hence $I^n / I^n \cong A/I$ for all $n$. This proves the claim.

Assume then that $r>1$ and that we have proven the result for smaller $r$. If $\gr_I (A)$ would not be isomorphic to the polynomial algebra, then we would have a homogeneous polynomial $F \in (A/I) [x_1,...,x_n]$ of degree $n$ such that $F([a_1],...,[a_r]) = 0$. This can clearly be lifted into a homogeneous polynomial $\widetilde{F} \in A[x_1,...,x_r]$ such that $\widetilde F (a_1,...,a_r) = 0$., because for any lift $F'$ of $F$ we must at least have $F' (a_1,...,a_r) \in I^{n+1}$. As $a_1$ is not a zerodivisor, we may assume that 
\end{proof}
...
Assume that this is not the case, i.e., we have some nonzero homogeneous $F$ in $K[x_1,...,x_d]$ of degree $n$ such that $F(a_1,...,a_d) = 0$. We can lift the coefficients of $F$ from $K$ to $A$ and obtain a homogeneous polynomial $\widetilde F \in A [x]$ such that $\widetilde F (a_1,...,a_d) \in m_A^{n+1}$. Thus for some homogeneous polynomial $H$ of degree $n+1$, we have that $(\widetilde F - H) (a_1,...,a_d) = 0$, i.e., we have a nontrivial algebraic relation among the $a_i$. We also see that the reduction $\overline{f} \in K[x_1,...,x_d]$ is not zero.

But this leads to a contradiction. If $x_1$ divides $f$, then we divide it by $x_1$ until it is no longer divisible by $x_1$, and obtain a new polynomial $f'$. As $a_1$ is not a zerodivisor, we must have $f'(a_1,...,a_d) = 0$. Otherwise we move to modulo $a_1$ and to a new polynomial $f(0,x_2,...,x_d)$. Then we do the same thing for $a_2$ and $a_3$ and so on, until we find that some $a_{i}$ is a zerodivisor modulo $a_1,...,a_{i-1}$. This must happen at least when $i = d$, as the polynomial $f$ had no constant terms. This concludes the proof.
\end{proof}
...
\end{equation*}
i.e., it is the projective scheme associated to the associated graded ring $\gr_I (A)$. But by the proof of \fref{AssociatedPolynomials}, we know that this is isomorphic to some polynomial algebra $(A/I)[x_1,...,x_r]$, and as $A/I$ is known to be regular, we conclude that the projective scheme $\proj ((A/I)[x_1,...,x_r])$ is regular as well.
\end{proof}
