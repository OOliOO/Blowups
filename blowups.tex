...
\end{lem}
\begin{proof}
We can pass to local coordinates, and assume that $X = \spec (k[x_1,...,x_n] / I)$. A closed point $p$ of $X$ can be represented as an $n$-tuple $(a_1,...,a_n)$ of elements of the algebraic closure $\overline{k}$ of $k$ satisfying all the algebraic relations defined by elements of $I$ (all tuples $(\sigma a_1,..., \sigma a_n)$, where $\sigma \in \mathrm{Gal}(\overline{k}/k)$, represent the same point, and these are all the tuples representing that point). If $f_i (x)$ are the minimum polynomials of $a_i$, then the maximal ideal of $p$ is generated by $f_i (x_i)$. Moreover, as $f_i (x_i)$ form a regular sequence in $k[x_1,...,x_n]$, and as $k[x_1,...,x_n]/I$ is regular at $p$, we may pick a subsequence which gives a regular sequence in $k[x_1,...,x_n]/I$ cutting out $p$. Without loss of generality we may assume this to be $f_1 (x_1),...,f_r(x_r)$. From now on we shall denote $f_i(x_i)$ just by $f_i$.

As $\partial_i f_i$ does not vanish at $p$, we may define the differential operators
\begin{equation*}
D_1 = {1 \over \partial_1 f_1} \partial_1, ... , D_r = {1 \over \partial_r f_r} \partial_r
\end{equation*}
at $p$. Clearly these generate all the $k$-derivations of $X$ at $p$. Moreover, these satisfy $D_i(f_j) = \delta_{ij}$. As the completed local ring at $p$ is just a power series in $f_1,...,f_r$ over the residue field $k[x_1,...,x_m]$
\end{proof}
