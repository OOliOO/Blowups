...
\end{prop}
\begin{proof}
It is clearly enough only to take care of the $r = 1$ case. We want to show that the natural map $A \left[ {a \over b} \right] \to B$ is an isomorphism. But as both of these are coincide with the coordinate ring of the second chart of $\proj (A [at, bt])$, we are done.
\end{proof}
...
Finally, just to be able to sleep well, we prove the following property.

\begin{prop}
Let $A$ be a ring, and $a_1,b_1,a_2,b_2 \in A$ such that $a_1b_2 = a_2b_1$. Now
\begin{equation*}
A \left[ {a_1 \over b_1} \right] = A \left[ {a_2 \over b_2} \right].
\end{equation*}
\end{prop}
\begin{proof}
It is enough to show that $b_2$ uniquely divides $a_2$ in $A \left[ {a_1 \over b_1} \right]$ as then we get a canonical map $A \left[ {a_2 \over b_2} \right] \to A \left[ {a_1 \over b_1} \right]$, whose inverse map is the canonical map in the other direction (obtained by a symmetric argument). Therefore the rings would be canonically isomorphic.

As $a_1b_2 = a_2b_1$, we see that $(a_1 / b_1) b_2 = a_2$, i.e., $a_2$ is at least divisible by $b_2$. The only thing left is to make sure that $b_2$ is not a zerodivisor. If this wasn't the case, then we'd have an $A$-polynomial $f$ such that
\begin{equation*}
f \left( {a_1 \over b_1} \right) b_2 = 0.
\end{equation*}
Again, for some large enough $n$, the product $f \left( {a_1 / b_1} \right) b_2^n$ corresponds to a homogeneous polynomial $F(a_2, b_2)$. NOPE
\end{proof}
