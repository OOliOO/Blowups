...
The proof strikes out as very odd. It is almost as if, purely by accident, the resolution just sort of happens. And we don't seem to be using the fact that we are turning the ideal $\mathscr{I}$ into a monomial ideal at all. We will get back to these points later.
 
\subsection{Monomalization of ideals -- the strategy of the proof}

In this section we outline the battle plan. Let us have a smooth $k$-variety $X$ and a sheaf of ideals $\mathscr{I}$. Instead of trying to directly associate to each pair $(X, \mathscr{I})$ a ''good'' map $X' \to X$ such that the total transformation $\mathscr{I}$ (the pre-image ideal sheaf) is monomial, it is beneficial for the inductive argument to associate a sequence of smooth blow-ups
\begin{equation*}
X_r \stackrel{\pi_{r-1}}{\to} \cdots \stackrel{\pi_0}{\to} X_0.
\end{equation*}
The centres of the blow-ups are considered to be part of the data, as they are not necessarily uniquely determined by the map (for example if we blow up a codimension one subvariety).
