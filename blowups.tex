...
The order of $\mathscr{I}$ actually drops by one at \emph{all} points, not just the closed ones, but to prove this, we need to see what the order at a generic point actually means. Recall that a generic point $\eta$ represents some irreducible subvariety $Y$ of $X$ (its closure). Under some mild assumptions, it actually holds that the order of $\mathscr{I}$ at $\eta$ is the smallest of the orders of $I$ at the closed points $y$ of $Y$. 

To give a more algebraic interpretation of the problem we pass to local coordinates. Assume that $X = \spec (A)$, where $A$ is a finitely generated $k$-algebra. Now $Y$ corresponds to some prime ideal $p \subset A$, and ideal $I \subset A$ having an order $n$ at $\eta$ (or \emph{along} $Y$) is clearly equal to $n$ being the largest integer such that the $n^{th}$ symbolic power $p^{(n)}$ of $p$ contains $I$. 

Assume that $I$ has at order of at least $n$ at all closed points of $Y$. Algebraically this means that $I$ is contained in the intersection $\bigcap_m m^{(n)}$, where $m$ runs over all the maximal ideals $m$ of $A$ containing $p$. As the normal and the symbolic powers coincide for maximal ideals, this is equivalent to $I \subset \bigcap_m m^n$. To prove our claim, i.e., that the order of $I$ at $\eta$ equals the smallest of the orders of $I$ at the closed points of the closure of $\eta$, it is enough to prove that
\begin{equation*}
p^{(n)} = \cap_{m \supset p} m^n.
\end{equation*}
We will need a couple of lemmas. The inclusion is easier, as we will see shortly.

\begin{lem}
Let $A$ be a regular $k$-algebra, and let $p \subset A$ be a prime ideal. Now 
\begin{equation*}
p^{(n)} \subset \bigcap_m m^n,
\end{equation*}
where $m$ runs over all maximal ideals of $A$ containing $p$.
\end{lem}
\begin{proof}
It is enough to prove that if $m$ is a maximal ideal of $A$ containing $p$, then we have $p^{(n)} \subset m^n$. If we denote by $K$ the residue field $A / m$ at $m$, then 
\end{proof}
